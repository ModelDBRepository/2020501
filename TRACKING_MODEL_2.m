%% Code to implement second tracking model.  
clear all
clc 
close all
rng(1) %fix rng 
...
I = IPSC + BIAS + WNV1*NOV*(dt*i>tcrit1)*(dt*i<tcrit2) + WNV2*I_INT(i); %implement + home cage changes 
...
% Store calcium responses for all neurons.  
if mod(i,nq)==1
kd = kd + 1;
REC2(kd,:) = ro; 
end
